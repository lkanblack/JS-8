    ...
    anchors.forEach(function(item) {
      ...
      const sendForm = () => {
        ...
        form.addEventListener('submit', (event) => {
            event.preventDefault(); // отменяем перезагрузки страницы при нажатии на кнопку
            form.appendChild(statusMessage);
            statusMessage.textContent  = loadMessage;
            // находим данные которые будет отправялть на сервер , передаем в функцию ту форму с которой хотим получить данные 
            const formData = new FormData(form);

            // отправка данных на серверв в JSON формате вместо form-data
            let body = {}; // то что мы будем отправлять на сервер 
            // достаем занчения из form data
            formData.forEach((val, key) => {
                body[key] = val; // добавляем в оъект body 
            });
            postData(body, ()=>{
        ...
        const postData = (body, outputData, errorData) => {
            const request = new XMLHttpRequest(); // начинаем писать запрос к серверу 
            // ставим имеено сюда для того чтобы могли отслеживать каждое состояние от загрузки до спасибо
            // оповещаем пользователя если все ок / для этого использует readyStateChange
            request.addEventListener('readystatechange', () => {
                // нужно дождаться readystatу 4 и requestStatus  200

                if(request.readyState !== 4){
                    return; // если не равен 4 то мы просто не идем дальше
                }
            ...
            // настраивем соединени - 1.Метод отправки(POST - отправляем данные на сервер)
            // 2 с помощью чего будем постить 
            request.open('POST', './server.php');
            // настройка заголовков - 1. имя заголовка - ContentType 2. - само значение/заголовок,указываем что данные отправляем с формы multipart/form-data / если JSON формат то appl
            request.setRequestHeader('Content-Type', 'multipart/form-data');
            
            // открываем соединение и отправляем данные с помощью метода send 
            // чтобы отправить на серв переводим в формат JSON 
